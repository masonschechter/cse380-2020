...
int main() {
    ...
    solution = Gauss_Seidel(DISCRETIZATION_POINTS, matrix, t0, RHS, 1e-11, 250000, "INFO");
    analytical_sol = analytical_solution_1D(DISCRETIZATION_POINTS, X_MIN, X_MAX);

    output_to_file(DISCRETIZATION_POINTS, 1, "VERIFICATION", X_MIN, X_MAX, solution, analytical_sol, "gauss_seidel_1D_4th_order_test_out.h5");
