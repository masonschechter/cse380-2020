...
int main() {
	...
	#ifdef PETSC_INCLUDE
		PetscErrorCode ierr = petsc_pipeline(inputs, timer);
		timer.Finalize();
		timer.Summarize();
		return 0;
	#endif
	...
	timer.Summarize();
	if (inputs.RUN_MODE == "VERIFICATION") {
		output_to_file(inputs.DISCRETIZATION_POINTS, inputs.DIMENSIONS, inputs.RUN_MODE, inputs.X_MIN, inputs.X_MAX, numerical_solution, analytical_solution, "sol.h5");
	} else {
		output_to_file(inputs.DISCRETIZATION_POINTS, inputs.DIMENSIONS, inputs.RUN_MODE, inputs.X_MIN, inputs.X_MAX, numerical_solution, NULL, "sol.h5");
	}
